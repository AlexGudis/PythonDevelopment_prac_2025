...
class client(cmd.Cmd):
    ...
    def addmon_params_check(self, args):
        ...
        try:
            ...
            print(
                'Something wrong with hello string when trying to parse it to a monster')
            raise ValueError
    ...
    def do_locale(self, loca):
        if loca in ['en_US.UTF-8', 'ru_RU.UTF8']:
            self.s.sendall(f"locale {loca}\n".encode())
        else:
            print('Choose another locale')
            return
        
    def do_attack(self, args):
        if len(args) == 0:
            print(
                'Invalid input. You should provide at least name of the monster to attack')
            return
...
if __name__ == '__main__':
    ...
    if s.recv(1024).rstrip().decode() == '1':
        ...
        print("This login is already assigned")
