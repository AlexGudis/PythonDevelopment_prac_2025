...
class MUD:
    ...
    def do_addmon(self, x, y, hp, hello, name):
        ...
        ans = {'name': name, 'x': x, 'y': y,'hello': hello}
    ...
    def do_movemonsters(self, args, player):
        global movemonsters
        locale.setlocale(locale.LC_ALL, player.lang)
            ...
            if request is send:
                ...
                match request.result().decode().split():
                    case ['addmon', *args]:
                        ...
                        await send_all(fun='addmon', args=game.do_addmon(int(x), int(y), int(hp), hello, name))
                    ...
                    case ['attack', *args]:
                        ...
                        weapon, name = args
                        result = game.do_attack(x, y, int(game.weapons[weapon]), name)
                        result['login'] = login
                    ...
                    case ['locale', loc]:
                        writer.write(players[login].set_locale(loc).encode())
            ...
            while not_done:
                ...
                if ( (random_m_x + dx) % 10, (random_m_y + dy) % 10) not in game.monsters_coords:
                    ...
                    for player in players.values():
                        locale.setlocale(locale.LC_ALL, player.lang)
                        lang = player.lang[0]  # 'en_US' или 'ru_RU'
                    ...
                    '''print('monster go direct')
                    await send_all(_("{name} moved one cell {direction}").format(name=random_monstr.name, direction=direction))
                    print('monster go coords')
                    await send_all(_("{name} new coords is {x}, {y}").format(name=random_monstr.name, x=random_monstr.x, y=random_monstr.y))
                    '''
